...
import { useByeRounds } from '../../src/hooks/usePlayers';
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.header}>
        ...
        {player.injury_suspension_status && (
          <View style={styles.injBanner}>
            <Text style={styles.injText}>{player.injury_suspension_status_text}</Text>
          </View>
        )}
      </View>
      ...
      <View style={styles.statsGrid}>
        ...
        <StatBox label="Season Avg" value={stats.avg.toFixed(1)} large />
        <StatBox label="L3 Avg" value={stats.avg3.toFixed(1)} large highlight />
        <StatBox label="L5 Avg" value={stats.avg5.toFixed(1)} large />
      </View>
      ...
      <View style={styles.priceRow}>
        <View style={styles.priceBox}>
          ...
          <Text style={styles.priceValue}>{formatPrice(stats.price)}</Text>
        </View>
        <View style={styles.priceBox}>
        ...
        <View style={styles.priceBox}>
          ...
          <Text style={[styles.priceValue,
            stats.ppts > stats.avg3 ? styles.down : styles.up
          ]}>
            {stats.ppts}
          </Text>
        </View>
        <View style={styles.priceBox}>
          ...
          <Text style={styles.priceValue}>{stats.owned.toFixed(1)}%</Text>
        </View>
      </View>
        ...
        <View style={styles.matchupRow}>
          ...
          <View style={styles.matchupItem}>
            ...
            <Text style={[styles.matchupValue,
              stats.oppavg > 75 ? styles.up : stats.oppavg < 60 ? styles.down : styles.neutral
            ]}>
              {stats.oppavg.toFixed(0)}
            </Text>
          </View>
          <View style={styles.matchupItem}>
          ...
          <View style={styles.matchupItem}>
            ...
            <Text style={styles.matchupValue}>{stats.venavg.toFixed(0)}</Text>
          </View>
        </View>
      </View>
      ...
      {stats.games > 0 && breakdown.length > 0 && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Score Breakdown — Round {CURRENT_ROUND}</Text>
          {breakdown.map(item => (
            <View key={item.stat} style={styles.breakdownRow}>
              ...
              <Text style={[styles.breakdownPts,
                item.points >= 0 ? styles.up : styles.down
              ]}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.prevSeason}>
          ...
          <StatBox label="Average" value={player.previous_average.toFixed(1)} highlight />
...
function StatBox({ label, value, large, highlight }: {
  ...
  return (
    <View style={[boxStyles.box, highlight && boxStyles.highlight]}>
      <Text style={[boxStyles.value, large && boxStyles.valueLarge, highlight && boxStyles.valueHighlight]}>
        {value}
      </Text>
...
const styles = StyleSheet.create({
  ...
  breakdownCount: { width: 28, fontSize: 11, color: COLORS.textMuted, textAlign: 'right' },
  breakdownBarTrack: { flex: 1, height: 6, backgroundColor: COLORS.border, borderRadius: 3, overflow: 'hidden' },
  breakdownBar: { height: '100%', borderRadius: 3 },
  breakdownPts: { width: 36, fontSize: 12, fontWeight: '700', textAlign: 'right' },
  prevSeason: { flexDirection: 'row', gap: 8 },
});
